...
\begin{figure}[h!]
   ...
   \includegraphics[width=350pt]{bilder/struktur2.pdf}
	\caption{Beispiel für Vereinigung, kartesisches Produkt, $r-$fache Vereinigung und $r-$fache Verschmelzung}
  	 ...
  	 \end{figure}
\end{bsp}
\begin{lem}(Bekannte Ergebnisse zur Vereinigung und zum kartesischen Produkt \cite{somefamiliesofgraphs})\newline
...
\IF{($u$ nicht markiert)}{
\STATE Ordne jeden Knoten $u$ mit $deg(u)\geq 4$ zu $\text{Klasse}[u]=A$;
\STATE Ordne jeden Knoten $u$ mit $deg(u)\leq 2$ zu $\text{Klasse}[u]=0$;
...
\end{algorithm}
\begin{algorithm}
\caption{Einteilung der zweifachen ZK in drei Typen (K, S und US)}
\begin{algorithmic}[1]
...
\IF{($x.Knoten==S[x]$)}{\STATE $x=S$;}\ELSIF{($x.Knoten==K[x]$)}{\STATE $x=K$;}\ELSE{\STATE $x=US$;}\ENDIF
}\ENDFOR
\vspace{2mm}
\end{algorithmic}
\label{algeinteilung}
\end{algorithm}
\begin{algorithm}
\caption{Einteilung und Berechnung der metrischen Dimension von den ZK}
\begin{algorithmic}[1]
\FORALL{({$x \in ZK$})}{
\STATE Bestimme die Anzahl der Knoten $u$ mit $Klasse[u]=A$ und speichere diese unter $Anker[x]$;
\IF {($x==C$)}{ \STATE $MD^+(x)=max\{0,x.Knoten-Anker[x]-1\}$;}\ELSIF {($x==K$)}{ \STATE $K(x)$;} \ELSIF {($x==S$)}{ \STATE $S(x)$;} 
\ELSIF {($x==US$)}{ 
	\IF{($Anker[x] \leq 2$ oder $Anker[x] \geq \frac{n}{2}$)}{
...
\ENDIF
\FORALL{($x \in v.ZK$)}{
\IF{($Anker[x]==1$ und $(n\; \textbf{mod} \; 2)==1$) oder\\$\;\;\;\;$($x==S$ und $(n\; \textbf{mod} \; 2)==0$) oder\\$\;\;\;\;$($x==S$ und $Anker[x]\geq 3$) oder\\$\;\;\;\;$($dist(v,x) \leq \lceil\frac{n}{2}\rceil$)oder\\$\;\;\;\;$($x==C$)}{\STATE $delete(x)$;}\ENDIF
}\ENDFOR
%}\ENDIF
