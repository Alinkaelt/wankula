...
\subsection{Glossar zum Kapitel}
...
Eine zweifache Zusammenhangskomponente beinhaltet immer einen Kreis und kann an den Knoten einfache Wege haben:
\begin{itemize}
\item[K] Dieser Knoten hat nur Nachbaren aus der gleichen zweifachen Zusammenhangskomponente.
\item[S] Dieser Knoten hat drei Nachbaren, ein Nachbar liegt an einem einfachen Weg.
\item[US] Dieser Knoten ist ein Bifurkator für jedes Knotenpaar aus jeder zweifachen Zusammenhangskomponente in welcher er liegt und mindestens einem Ankerknoten. 
\end{itemize}
\end{defi}
...
\vspace{-6mm}
  	   	  	   	 \begin{figure}[h!]
		\centering
 		 \includegraphics[width=320pt]{bilder/kaktusallgschritt8.pdf}
...
\begin{itemize}
\item Der Algorithmus \ref{alginit} bestimmt zunächst alle zweifachen Zusammenhangskomponenten und speichert sie in einer Liste von Listen, sofern diese aus mindestens drei Kanten besteht. Die asymptotische Laufzeit von diesem Algorithmus ist $\mathcal{O}(|V|+|E|)$. \newline
In Zeile 5-7 wird die Darstellung der zweifachen Zusammenhangskomponenten von Kanten auf Knoten geändert. Dafür würde man über alle zweifachen Zusammenhangskomponenten und über jeweils die Kanten in einer zweifachen Zusammenhangskomponente laufen und die Anzahl von Kanten zählen. Die inzidenten Knoten zu der Kante werden in eine Zusammenhangskomponente aufgenommen, sofern diese noch nicht drin sind, und die Kante aus der ZZK gelöscht. Die unterschiedlichen Knoten $v$ in einer ZZK werden gezählt und $ZK(v)=1$ gesetzt. Nach der Definition eines Kaktusgraphen darf keine Kante in zwei Kreisen und somit in zwei zweifachen Zusammenhangskomponenten liegen. Der Algorithmus läuft über eine Teilmenge der Kanten einmalig drüber und hat eine asymptotische Laufzeit von $\mathcal{O}(|V|+|E|)$.\newline
Die Anweisungen in den Zeilen 8-11 sind für die Einteilung von Knoten aus ZZK in Klassen zuständig. Für Knoten vom Grad drei wird mittels Tiefensuche ermittelt ob ein Weg nur über Knoten vom Grad zwei erreichbar ist. Sofern dies zutrifft ist die Länge $i\geq 1$ von diesem Weg die Klasse $i$ des betrachteten Knotens. Ansonsten ist seine Klasse $A$. Jeder Knoten wird maximal einmal besucht. Ein weiterer Durchlauf über die restlichen Knoten ist notwendig um die restlichen Knoten in Abhängigkeit von ihrem Grad einer Klasse zu zuordnen. Die asymptotische Laufzeit von dem Abschnitt ist in $\mathcal{O}(|V|+|E|)$.\newline
In der Zeile 12 werden nochmal Knoten der Klasse $A$  mit $deg(v)\geq 4$ überprüft ob ein Blatt nur über Knoten vom Grad zwei erreichbar ist. Man iteriert über eine Teilmenge der Knoten und benutzt die Tiefensuche. Die asymptotische Laufzeit hier ist ebenfalls in $\mathcal{O}(|V|+|E|)$ und damit linear für den gesamten Algorithmus.

\item Der Algorithmus \ref{algeinteilung} überprüft den Typ einer zweifachen Zusammenhangskomponente und ruft einen entsprechenden Algorithmus zur Berechnung der metrischen Dimension auf. Es gibt zwei \textsc{for}-Schleifen in Zeile 1 und 2 über alle zweifachen Zusammenhangskomponenten und die Knoten in jeder ZZK. Über jeden Knoten $v$ wird maximal $\frac{\mathcal{N}(v)}{2}$-mal iteriert, also über die Hälfte seiner Kanten. Die asymptotische Laufzeit ist damit in $\mathcal{O}(|V|+|E|)$. Dannach wird mittels einer Fallunterscheidung einer von den Algorithmen \ref{algberechnungsonne}, \ref{algberechnungkreis},\ref{algberechnungungeradesonne1}, \ref{algberechnunggeradesonne1}, \ref{algberechnungusonne2} oder \ref{algberechnungungeradesonne0} aufgerufen.

\item Der Algorithmus \ref{algsonderfall1} läuft einmal am Anfang über jeden Knoten in jeder zweifachen Zusammenhangskomponente und speichert dabei den Abstand zwischen zwei Knoten der Klasse $A$. An jedem $A$ Knoten wird der größere von den beiden Abständen zu dem nächsten $A$ Knoten gespeichert und an jeder zweifachen Zusammenhangskomponente der größte Abstand zwischen zwei $A$ Knoten in dieser Komponente.
\begin{figure}[h!]
\centering
\includegraphics[width=140pt]{bilder/bspalgsonderfall2.pdf}
...
\caption{Verlauf für die Berechnung der MD einer geraden Sonne ohne $A$ Knoten}
\end{figure}
...
\begin{lem}
Ein Kaktusgraph ist genau dann getrennt, wenn die Knoten in jeder Komponente (Baumkomponente oder zweifachen Zusammenhangskomponente mit oder ohne einfache Wege) getrennt sind und wenn die Nachbarschaft von allen Amalgamationsknoten getrennt ist.  
\end{lem}
\begin{proof}[Beweis:]~\newline
...
$\Longleftarrow$ Nach Lemma \newline

Nach Lemma \ref{first_theorem} und der Verallgemeinerung gilt für jedes Knotenpaar in unterschiedlichen Teilgraphen welche nicht\newline
...
\end{proof}
\begin{bem}
Der Grund dafür das einfache Wege nicht als Komponente aufgefasst werden müssen ist abhängig von der Position des Weges. Die Wege an zweifachen Zusammenhangskomponenten werden bei der Berechnung der metrischen Dimension der Komponenten beachtet. Wege zwischen zwei Komponenten sind nach Lemma \ref{} 
\end{bem}
Einer der Gründe weshalb es keinen einfachen und effizienten Algorithmen für die Berechnung der metrischen Dimension gibt, ist dass sich das Problem nicht lokal auf Teilgraphen lösen lässt und die metrische Dimension für den Gesamtgraphen nicht einfach die Summe der Komponenten ist (vgl. Kapitel \ref{kapallg}).\newline\newline
Die Kaktusgraphen haben durch die starken Einschränkungen der Struktur nur ''eine handvoll'' unterschiedlicher Komponenten als Bestandteile, welche durch jeweils einen Trennungsknoten verbunden sind. Dazu gehören Bäume, Wege und Kreise.\newline\newline
Nach dem Lemma \ref{wegtrennungsknoten} kann ein Trennungsknoten lokal als Ankerknoten aufgefasst werden. Hierbei bedeutet lokal, dass man nur einen Teilgraph mit dem Trennungsknoten als Gesamtgraph betrachtet. Aus dem Lemma \ref{einelementreichtnicht} ist bekannt, dass ein Teilgraph, welcher kein Kreis ist immer mindestens einen Ankerknoten beinhaltet. Daraus folgt aber nicht, dass Wege keine Ankerknoten haben können. Es bedarf einer gesonderten Untersuchung wie sich Wege als Teilgraphen verhalten. Ein Weg kann an einem Baum vorkommen, es bleibt aber für die Untersuchung ein Baum. Außerdem kann ein Weg an einem Kreis vorkommen. Es entsteht dadurch eine voll- oder unvollständige Sonne. Der Fall, dass mehrere Kreise und ein Weg an einem Knoten vorkommen wird ebenfalls untersucht. Weitere Möglichkeiten gibt es nicht. \newline
...
\IF{(Anzahl der Kanten $\geq 3$)}{\STATE Speichere sie als eine Komponente $x$ in der Liste $ZK$;} \ENDIF
...
%\IF{($u$ nicht markiert)}{
\STATE Ordne jeden Knoten $u$ mit $deg(u)\geq 4$ und $ZK[u]=1$ zu $\text{Klasse}[u]=A$;
\STATE Ordne jeden Knoten $u$ mit $deg(u)\leq 2$ und $ZK[u]=1$ zu $\text{Klasse}[u]=0$;
%\STATE Ordne jeden Knoten $u$ mit $deg(u)=3$ und $ZK(u)=0$ zu $\text{Klasse}[u]=0$;
...
\IF{($Klasse[u]==A$ und $deg(u)\geq 4$ und $ZK[u]==1$)} \STATE Überprüfe mit einer Tiefensuche ob man über Knoten vom Grad zwei zu einem Blatt kommt und setze $Weg[u]=1$. Falls nicht, setze $Weg[u]=0$; \ENDIF}
\ENDFOR
\end{algorithmic}
\label{alginit}
\end{algorithm}
...
\IF{($x.Knoten==S[x]$)}{\STATE $S(x)$;}\ELSIF{($x.Knoten==K[x]$)}{\STATE $K(x)$;}\ELSE{\IF{($Anker[x] \leq 2$ oder $Anker[x] \geq \frac{n}{2}$)}{
		...
		{		 
			\IF{($Anker[x]==0$)}{\STATE $USU0(x)$;}
			\ELSE{\STATE $USU1(x)$;}
			\ENDIF}
		\ELSIF{($(n\; \textbf{mod} \; 2)==0$)}{\STATE $USG(x)$;
		}\ENDIF}
	\ELSE{\STATE $US(x)$;
	}\ENDIF}\ENDIF
}\ENDFOR
...
\FORALL{($v \in V$)}{
\STATE $create(v.ZK)$;
\IF{($Klasse[v]==A$ und $ZK[v]==1$ und $deg(v)\geq 4$)}{
\STATE $insert(v.ZK \in V.ZK)$;}
\ELSE{ \STATE $delete(v.ZK)$;}
\ENDIF
%\IF{($Klasse[v]==A$ und $ZK[v]==1$ und $deg(v)\geq 4$)}{
%\FORALL{($x \in ZK$)}{
%\IF{($v \in x$)}{
%\STATE $insert(x \in v.ZK)$;}
%\ENDIF
%}\ENDFOR
\IF{($Weg[v]==1$)}{
\STATE $insert(v \in v.ZK)$;}
\ENDIF
\FORALL{($x \in v.ZK$)}{
...
%}\ENDIF
\IF{($|v.ZK|\leq 1$)}\STATE $delete(v.ZK)$;\ENDIF
}\ENDFOR
\end{algorithmic}
...
\begin{algorithmic}[1]
\vspace{2mm} 
\IF{($Anker[x]==1$ und $n>4$)}\STATE $a=0$; \STATE Finde maximale zusammenhängende Menge links $M$ mit $|M|=s$ und rechts $M'$ mit $|M'|=s'$ mit $Klasse[v]=0$;
\IF{($s\geq \frac{n}{2}-2$ oder $s'\geq \frac{n}{2}-2$)}{ \STATE $MD^+(x)=1$;$a=1$;}\ENDIF
\STATE Finde maximale gegenüberliegende zusammenhängende Menge links $M$ mit $|M|=s$ und rechts $M'$ mit $|M'|=s'$ mit $Klasse[v]=0$;
\IF{(Einer der beiden Fälle die unvollständige Sonne trennt)}{ \STATE $MD^+(x)=1$; $a=1$;}\ENDIF
\IF{($a==0$)} \STATE $MD^+(x)=2$;\STATE $insert(x \in MD-)$;\ENDIF
\ELSIF{($Anker[x]\geq \frac{n}{2}-2$)} \STATE $MD^+(x)=0$; \STATE $insert(x \in MD-)$;\ELSIF{($Anker[x]==2$ und $n>4$)} \IF{(Die unvollständige Sonne getrennt wird)}{ \STATE $MD^+(x)=0$;}\ELSE{\STATE $MD^+(x)=1$;\STATE $insert(x \in MD-)$;}\ENDIF \ENDIF
\vspace{2mm}
\end{algorithmic}
...
\begin{algorithmic}[1]
\vspace{2mm}
\IF{($dist[x]\leq \lceil\frac{n}{2}\rceil$)}{\STATE $MD^+(x)=0$;\STATE $insert(x \in MD-)$;}\ELSIF{(Jeder Knoten $v \in kw(v_1, \; v_{2,\frac{n}{2}}) \setminus \{v_1, \;v_{2,\frac{n}{2}}\}$ und $v \in kw(v_2, \; v_{1,\frac{n}{2}}) \setminus \{v_2, \;v_{1,\frac{n}{2}}\}$ hat $Klasse[v]=0$)}{ \STATE $MD^+(x)=0$; \STATE $x \in CHECK$;}
...
\begin{algorithmic}[1]
\vspace{2mm} 
\IF{($Anker[x]==0$ und $n>4$)} \STATE $a=0$; Finde maximale zusammenhängende Menge $M$ mit $Klasse[v]=A$;
\IF{($|M| \geq \frac{n}{2}-2$)} \STATE $MD^+(x)=2$; $a=1$;\ENDIF
...
\IF{(Ein Intervall bleibt bis zum Ende in der Liste)}{ \STATE $MD^+(x)=2$; $a=1$;}\ENDIF
\IF{($a==0$)} \STATE $MD^+(x)=3$; \ENDIF \ENDIF
\vspace{2mm}
\end{algorithmic}
