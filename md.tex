...
\begin{lem}
\label{usg}
Ist $max\;dist(r_i,r_j)> \lceil \frac{n+2}{2}\rceil$ kann der unvollständige Sonnengraph getrennt sein. Ist der Sonnergraph ungerade, so ist er durch zwei Ankerknoten getrennt oder durch drei Ankerknoten, wenn er gerade ist und es keine Strahlen auf allen Knoten $v$ gilt mit:\newline
...
\IF{($Anker[x]==1$ und $n>4$)}
	...
	\IF{($\{v_1, v_{1-s-1+\frac{n}{2}}\}\text{ oder }\{v_1, v_{1+s'+1+\frac{n}{2}}\}$ die ZSK trennen)}{\STATE $MD^+(x)=1$;}\ELSE{\STATE $MD^+(x)=2$;\STATE $insert(x \text{ in }  MD-)$;}\ENDIF
	}\ENDIF
