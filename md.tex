...
\begin{proof}[Beweis:]
Es gibt zwei nicht getrennte Knoten $u,v$ mit den Markierungen $a/b$ von den zwei Ankerknoten $r_1,r_2$ und $a-l/b-l$,$\;a-i/b-i$ für $0\leq i,l$ die Markierung der Strahlursprünge. Die Wege zwischen den Strahlursprüngen müssen knotendisjunkt sein, da es keinen Bifurkator gibt. Angenommen es gibt einen weiteren Ankerknoten $r_3$ und der Knoten $v$ bekommt die Markierung $a/b/c$ und $a\neq c$ und $b\neq c$, da es nur einen Ankerknoten pro Strahl gibt. Sei $r_3$ o.E.d.A. auf dem kürzesten Weg von $u$ zu $r_1$. Die Markierung von $v$ ist $a/b/c+2b=2a-c$. Für $dist(r_3,u)=dist(r_3,v)$ muss $a=c$ sein. $\lightning$ \end{proof}
\begin{lem}
...
\vspace{-1mm}
\begin{proof}[Beweis:]
Analog zum Beweis von Lemma \ref{ungerade_sonne} mit dem Unterschied, dass die maximal mögliche Distanz $\frac{n}{2}$ ist und dadurch der maximale Abstand zwischen zwei Ankerknoten größer als $n-\frac{n}{2}-1=\frac{n+2}{2}$ ist.
\end{proof}
...
$\cdot$ $v$ liegt zwischen den Knoten $r_i$ und $r_{j+\lfloor\frac{n}{2}\rfloor}$ (gezählt auf der Seite wo $r_i$ liegt) \\$\cdot$ $v$ liegt zwischen den Knoten $r_j$ und $r_{i+\lfloor\frac{n}{2}\rfloor}$ (gezählt auf der Seite wo $r_j$ liegt)
\end{lem}
\begin{bem}
Alle Knoten bei welchen es keinen Strahl geben darf, würden sonst Bifurkatoren zwischen allen Ankerknoten den Knoten auf dem Strahl und Knoten auf dem Kreis sein. Gibt es keine Knoten auf dem Strahl, so gibt es auch keine Bifurkatoren mit der Eigenschaft aus Lemma \ref{Bifurnachbar} nicht. Nach den Lemmata \ref{lemnurbifur1} und \ref{lemnurbifur2} können nur nicht getrennte Knotenpaare existieren, wenn es einen solchen Bifurkator gibt. Der unvollständige Sonnengraph ist damit getrennt.
\end{bem}
\begin{bem}
