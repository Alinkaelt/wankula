...
%
\section*{\ifthenelse{\equal{\sprache}{deutsch}}{Danksagung}{Acknowledgments}}
...
\vspace{-4mm}
~\linebreak
Von dieser Problematik motiviert veröffentlichte Slater im Jahre 1975 den Artikel \glqq{Leaves of Trees}\grqq$\;$\cite{slater} in dem es um seine Forschung an dem Langstreckennavigationssystem \glqq{U.S. Sonar}\grqq$\;$und Küstenwache \glqq{Loran}\grqq$\;$ging. Er stellte den Raum durch einen Graphen dar, den Agenten als einen Knoten und die Sendestationen als eine ausgewählte Menge von Knoten, welche er als ein \glqq{locating set}\grqq$\;$ bezeichnete. Die Kennung eines Knotens sollte die Distanzen der kürzesten Wege zu jedem ausgewählten Knoten sein.\vspace{-1mm}\newline\newline Unabhängig von Slater führten Harary und Melter in ihrer Arbeit \glqq{On the metric dimension of a graph}\grqq$\;$\cite{harary} im Jahre 1976 dasselbe Problem ein. Sie bezeichneten die minimalen Mengen von Knoten, durch welche jeder Knoten eine eindeutige Kennung bekommt, mit metrische Basis und die Anzahl dieser Knoten als die metrische Dimension eines Graphen.\vspace{-2mm}\newline\newline
Drei Jahre später gab es das erste Ergebnis zur Komplexität des Problems in dem Buch \glqq{Computers and Intractability: A Guide to the Theory of NP-Completeness}\grqq$\;$\cite{book}. Eine Bemerkung in dem Buch sagte aus, dass durch eine Reduktion von dem Problem 3DM gezeigt werden kann, dass das Problem ob ein gegebener Graph eine metrische Dimension kleiner $k$ hat NP-vollständig ist. Im Jahre 1993 wurde die metrische Dimension von Mark Johnson in seiner Arbeit \glqq{Structure-activity maps for visualizing the graph variables arising in drug design}\grqq$\;$\cite{drug} für den Umgang mit großen chemischen Daten entdeckt. Zu weiteren praktischen Anwendungen gehören u.a. Netzwerkerkennung z.B. aus \glqq{Network Discovery and Verification}\grqq$\;$\cite{netzwerk}, Umgang mit hierarchischen Datenstrukturen z.B. aus \glqq{Metric bases in digital geometry}\grqq$\;$\cite{hier} und die weiteren Arbeiten von Slater zur Lokalisierung als praktische Anwendungen der metrischen Dimension. Es gibt außerdem einen Zusammenhang zwischen dem Münzwiegeproblem, Mastermind Spielen und der metrischen Dimension von Hyperwürfeln in \glqq{On Metric Generators of Graphs}\grqq$\;$\cite{tannier}.\newpage

Seit der Einführung der metrischen Dimension wurde diese von zahlreichen Graphklassen bestimmt z.B. von Kreisen \cite{landmarks}, Radgraphen \cite{wheel}, vollständig bipartiten Graphen \cite{upper}, Gittern \cite{landmarks}, Honeycomb Netzwerken \cite{honey}, kreisartigen Netzwerken \cite{circulant} und Cayley Digraphen\cite{cayley}.\vspace{-1.5mm}\newline\newline Hervorzuheben sind auch die vollständigen Charakterisierungen von bestimmten metrischen Dimensionen, wie z.B. Wegen als einzige Graphen mit metrischer Dimension eins \cite{landmarks} oder vollständigen Graphen mit metrischer Dimension $|V|-1$ \cite{upper}. 
Eine vollständige Charakterisierung gibt es ebenfalls von Graphen mit metrischer Dimension $|V|-2$ \cite{upper} und $|V|-3$ (\cite{n-31,n-32}).\\
Es wurde eine weitere Reduktion der metrischer Dimension allgemeiner Graphen von 3SAT in \glqq{Landmarks in graphs}\grqq$\;$\cite{landmarks} angegeben. Außerdem konnte gezeigt werden, dass das Problem der metrischen Dimension u.a. für planare Graphen in \glqq{On the Complexity of Metric Dimension}\grqq$\;$\cite{aussenplanar} und bipartite Graphen in \glqq{An efficient representation of Benes networks and its applications}\grqq$\;$\cite{bipartitnp} ebenfalls NP-vollständig ist. 
Neben dem Linearzeit-Algorithmus für Bäume aus \glqq{On the metric dimension of a graph}\grqq$\;$\cite{harary} und \glqq{Landmarks in graphs}\grqq$\;$\cite{landmarks} konnte in \glqq{On the Complexity of the Metric Dimension}\grqq$\;$\cite{aussenplanar} gezeigt werden, dass sich die metrische Dimension von außenplanaren Graphen in Polynomialzeit berechnen lässt.\vspace{-1mm}\newline\newline
...
\begin{figure}[h!]
		\centering 		 
   \includegraphics[width=330pt]{bilder/vollstaendigergraph.pdf}
...
Links wurden zwei Ankerknoten ausgewählt, diese sind grau gezeichnet, aber ein Knotenpaar wird nicht getrennt. Dieses Knotenpaar wurde rot markiert. In dem mittleren Fall wurden drei Ankerknoten ausgewählt, der gesamte Graph ist getrennt und auf keinen Ankerknoten kann verzichtet werden. Daraus folgt, dass die metrische Dimension von diesem Graphen höchstens drei ist, aber auch kleiner sein kann. Die rechte Auswahl an Knoten zeigt, dass die metrische Dimension zwei ist. Das ist auch das mögliche Minimum, was der nachfolgende Abschnitt bestätigt.
\end{bsp}
\begin{bem}
...
Eine Übersicht der Ergebnisse zum kartesichen Produkt und zur Vereinigung von Graphen liefert die Arbeit \glqq{On the metric dimension of some families of graphs}\grqq$\;$\cite{families}. Eine weitere Verknüpfung von Graphen ist das Corona Produkt.
\begin{comment}
...
\begin{figure}[ht]
   ...
   \includegraphics[width=325pt]{bilder/struktur4.pdf}
	\caption{Beispiel für das Corona Produkt}
  	 ...
  	 ~\linebreak
\end{bsp}
Neben einigen interessanten Ergebnissen zum Corona Produkt wurde in  \glqq{On $k$-dimensional graphs and their bases
}\grqq$\;$\cite{bases} das folgende gezeigt:
...
\begin{lem}
\label{Bifurnachbar}
Seien ein Graph $G=(V,E)$ und eine Menge $R \subset V$ mit $R''=\{r_1, \ldots , r_n\}$, sowie auserwählte Knoten $v,x,y$ mit $dist(v,x)=dist(v,y)$ gegeben. Ist $v$ der Bifurkator von $r_i,x,y$ für $1 \leq i \leq n$, so ist $R''$ keine metrische Basis von dem Graphen $G$.
\end{lem}
\begin{proof}[Beweis:]
Angenommen $R''$ ist eine metrische Basis des Graphen $G$ und der Knoten $v$ besitze die Markierung $a_1/\ldots /a_n$. Sei die $dist(v,x)=b$. Da $v$ der Bifurkator ist, liegt er auf einem kürzesten Weg und so ist die Markierung vom Knoten $x$ $a_1+b/\ldots /a_n+b$. Nach der Voraussetzung ist $dist(v,x)=dist(v,y)$, damit hat der Knoten $y$ die Markierung $a_1+b/\ldots /a_n+b$ und die Knoten $x$ und $y$ sind nicht getrennt. Dies ist ein Widerspruch zur Definition der metrischen Basis. $\lightning$
\end{proof}
~ \linebreak
\vspace{-12mm}
\begin{lem}
...
\end{proof}
Betrachtet man nur den Teilgraphen $G_1 \cup \{x\}$ oder $G_2 \cup \{x\}$, so kann der Knoten $x$ als Ankerknoten betrachtet werden. Solche Betrachtungen werden als lokal bezeichnet.
\begin{lem}
...
Der zusammenhängende Graph $G_n$ entsteht durch das Verbinden der Knoten $u$ und $v$ durch einen Weg der Länge $n \geq 1$. Die Knoten auf dem Weg werden als $p_i$ mit $1 \leq i \leq n$ bezeichnet.\\
Für eine metrische Basis $R_k= \{ r_1, \ldots , r_k \}$ mit der Eigenschaft, dass es ein $r_i \in V_1$ und ein $r_j \in V_2$  mit $n \neq j$ gilt:
\[p_i \neq r_m \text{ für alle } 1 \leq m \leq k \text{ und } 1\leq i \leq n\]
\end{lem}
\begin{proof}[Beweis:]
Jeder Knoten auf dem Weg $p_i$ hat den gleichen Bifurkator für den Knoten $r_j$ und für jedes Knotenpaar in $G_1$ und das gleiche gilt für den Knoten $r_m$ und jedes Knotenpaar in $G_2$. Damit ist jeder Knoten auf dem Weg $p_i$ als Ankerknoten überflüssig.
\end{proof}
\begin{lem}
\label{nachbartrennungsknoten}
Sei ein beliebiger Graph $G=(V,E)$ gegeben. Seien $G_L$ und $G_R$ zwei Teilgraphen, die keine Wege sind, von $G$, die durch genau einen Separationsknoten $v_s$ verbunden sind. Gebe ein nicht getrenntes Knotenpaar $\{v_L,v_R\}$ mit den Eigenschaften, dass $v_L\in G_L$ und $v_R \in G_R$ und $v_L, v_R \notin \mathcal{N}(v_s)$.\newline
Dann ist das Knotenpaar $u_L,u_R \in \mathcal{N}(v_s)$, mit den Eigenschaften, dass $u_L$ auf dem kürzesten-Weg von $v_s$ zu $v_L$ liegt und $u_R$ auf dem kürzesten-Weg von $v_s$ zu $v_R$ liegt, nicht getrennt.
\end{lem}\vspace{-2mm}
\begin{proof}[Beweis:]~\newline
\vspace{-8mm}
\begin{figure}[!h]
...
\caption{Nachbarknoten von Separationsknoten $v_s$}
\end{figure}
~\linebreak
...
Sei ein beliebiger Graph $G=(V,E)$ gegeben. Seien $G_L$ und $G_R$ Teilgraphen von $G$, die keine Wege sind, und ein Weg $p$ durch genau einen Separationsknoten $v_s$ verbunden sind. Gibt es ein nicht getrenntes Knotenpaar $\{v_L,v_R\}$ mit der Eigenschaft, dass $v_L\in p$ und $v_R \in G_R$ und $v_L, v_R \notin \mathcal{N}(v_s)$, dann ist das Knotenpaar $u_L,u_R \in \mathcal{N}(v_s)$ mit der Eigenschaft, dass $u_L$ auf dem kürzesten-Weg von $v_s$ zu $v_L$ und $u_R$ auf dem kürzesten-Weg von $v_s$ zu $v_R$ liegt, nicht getrennt.
\end{lem}
\par
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Der Einfluss von Knotenkontraktion von Separationsknoten vom Grad zwei auf die metrische Dimension}
Gegeben seien zwei Graphen $G_1=(V_1,E_1)$ und $G_2=(V_2,E_2)$ und zwei Knoten $u,v$ mit $u \in V_1$ und $v\in V_2$.
Der zusammenhängende Graph $G$ entsteht durch das Verbinden der Knoten $u$ und $v$ durch einen Weg $p$ der Länge $n \geq 1$. Bei einer metrischen Basis $R_k$ mit $k \geq 2$ befindet sich mindestens ein Ankerknoten in jedem Teilgraphen $G_1$ und $G_2$. Nach Lemma \ref{keinknotenvonwegindermd} können nur Knoten aus den Teilgraphen $G_1$ und $G_2$ Ankerknoten sein. In diesem Abschnitt werden nur solche Graphen $G$ betrachtet.
...
\begin{figure}[ht]
\centering
\includegraphics*[width = 220pt]{bilder/proof4,2.pdf}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%BEWEIS%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{proof}[Beweis:]
Der Beweis ist analog zum Beweis zum Lemma \ref{lem2} unter der Voraussetzung das die metrische Dimension in $G'$ kleiner ist als in $G$.
\end{proof}
\begin{bem}
...
\end{bem}
\begin{lem}
Ist in einem Teilgraphen kein Ankerknoten enthalten, so kann durch die Kontraktion von Kanten, sofern ein inzidenter Knoten ein Separationsknoten vom Grad zwei ist, die metrische Dimension eines Graphen verkleinert werden.
\end{lem}
\begin{proof}[Beweis:] ~ \newline
\vspace{-7mm}
\begin{figure}[!h]
...
\label{chap_sonne}
In der Arbeit \glqq{On k-dimensional graphs and their bases}\grqq$\;$\cite{bases} wird die metrische Dimension des Corona Produktes von Kreisen $C_n$ und Wegen $P_1$ betrachtet. Diese Graphen sind in der Literatur auch unter dem Namen \glqq{sunlet}\grqq$\;$\cite{sunwebsite} bekannt. In diesem Kapitel wird gezeigt, dass ihr Resultat sich auf Wege beliebiger Länge übertragen lässt. Außerdem werden ebenfalls Teilgraphen der Sonnengraphen betrachtet, die unvollständigen Sonnengraphen. Beide Graphklassen sind sehr wichtig für den Algorithmus in Kapitel \ref{kapkaktus}.
...
Für $n=1$ ist der Graph der Sterngraph aus Definition \ref{defstern}. Als Sonnengraph $S_n$ bezeichnet man einen Graphen mit $n$ Strahlen beliebiger Länge $i$ mit $i \geq 1$. Ist $n=2m+1$ so nennt man den Sonnengraphen ungerade, ist $n=2m+2$ mit $m \in \mathbb{N}^+$ so bezeichnet man den Sonnengraphen als gerade.
\par
\end{defi}
\newpage
%\begin{bem}
...
Es gibt pro Strahl nur einen Ankerknoten. Das Vorziehen eines Endknotens gegenüber einem Strahlurprung als Ankerknoten im gleichen Strahl kann die metrische Dimension nicht vergrößern.
\end{lem}
\begin{proof}[Beweis:]
...
\begin{lem}
\label{lemnurbifur1}
Sei eine Sonne $S_n$ mit zwei Ankerknoten gegeben. Gibt es ein nicht getrenntes Knotenpaar, welches keinen gemeinsamen Bifurkator für je einen Ankerknoten und das Knotenpaar hat, so ist die Sonne gerade.
\end{lem}
\vspace{-4mm}
\begin{proof}[Beweis:]
\begin{enumerate}
\item Es gibt zwei Knoten auf dem Kreis, welche nicht getrennt sind. Dieser Fall tritt nur auf wenn der Kreis gerade ist und die Ankerknoten in gegenüberliegenden Strahlen sind.
\item Zwei Knoten, wobei mindestens einer in einem Strahl ist, sind nicht getrennt. Sei $a+k/b+k$ mit $a,\;b,\;k \geq 0$ die Markierung des Knoten in dem kürzeren Strahl. Die Markierung des Strahlursprungs ist $a/b$. Damit es einen weiteren nicht getrennten Knoten gibt muss es einen Knoten mit der Markierung $a+k/b+k$ geben und damit auch einen Strahlursprung mit der Markierung $a+l/b+l$ mit $1 \leq l \leq k$ geben. Sei o.B.d.A. $l\geq 1$, denn für $l=0$ tritt Fall eins auf. Die Wege zwischen dem Strahlursprung mit der Markierung $a/b$ und dem Strahlursprung mit der Markierung $a+l/b+l$ müssen knotendisjunkt sein, da es keinen Bifurkator gibt. Für die Größe des Kreises der Ursprungsknoten gilt: $a+l+b+l+a+b=2(a+b+l)$ und damit $2(a+b+l)\neq 2n+1$ für $a,\;b,\;l,\;n\in \mathbb{N}$. \end{enumerate}
\end{proof}

\vspace{-4mm}\begin{bem}
\label{bemungeradesonne}
Ein ungerader Kreis hat damit nur nicht getrennte Knotenpaare mit einem gemeinsamen Bifurkator für jeden Ankerknoten und das Knotenpaar.
\end{bem}

\begin{lem}
\label{lemnurbifur2}
Sei eine gerade Sonne $S_n$ gegeben. Gibt es ein nicht getrenntes Knotenpaar, welches keinen gemeinsamen Bifurkator für je einen Ankerknoten und das Knotenpaar hat, so gibt es zwei Ankerknoten.
\end{lem}

\begin{proof}[Beweis:]
Es gibt zwei nicht getrennte Knoten $u,v$ mit den Markierungen $a/b$ von den zwei Ankerknoten $r_1,r_2$ und $a-l/b-l$,$\;a-i/b-i$ für $i,\;l\geq 0$ die Markierung der Strahlursprünge. Die Wege zwischen den Strahlursprüngen müssen knotendisjunkt sein, da es keinen Bifurkator gibt. Angenommen es gibt einen weiteren Ankerknoten $r_3$ und der Knoten $v$ bekommt die Markierung $a/b/c$ und $a\neq c$ und $b\neq c$, da es nur einen Ankerknoten pro Strahl geben darf nach Lemma \ref{knotenimstrahl}. Sei $r_3$ o.E.d.A. auf dem kürzesten Weg von $u$ zu $r_1$. Die Markierung von $v$ ist $a/b/c+2b=2a-c$. Damit $dist(r_3,u)=dist(r_3,v)$ muss $2a-c=c$ und damit $a=c$ sein. $\lightning$ \end{proof}
\begin{lem}
\label{sonneerweiterung}
Sei ein Sonnengraph $S_{n}$ gegeben. Auf jeder Kante auf den Wegen kann mit Knoten vom Grad zwei erweitert werden und Kanten, sofern ein inzidenter Knoten den Grad zwei hat, auf den Wegen können ohne Auswirkungen auf die metrische Dimension kontrahiert werden.
\end{lem}
\begin{proof}[Beweis:]
Angenommen der $S_n$ sei durch eine Menge von Knoten nicht getrennt, durch welche der $S_{n,1}$ getrennt ist.
Sei $a+k/b+k$ für einen ungeraden Kreis und $a+k/b+k/c+k$ für einen geraden Kreis die metrischen Koordinaten von $v$. Angenommen, es gibt einen Knoten $u$ in dem Graphen $G$ mit den gleichen metrischen Koordinaten.
\begin{enumerate}
\item Der Strahlursprung $v_k$ hat die metrischen Koordinaten $a/b$ oder $a/b/c$. Sofern es einen anderen Knoten $v'$ als $v$ gibt mit der Distanz $k$ zu dem Knoten $v_k$ und den gleichen metrischen Koordinaten wie $v$, so gibt es auch einen Knoten zwischen den Knoten $v'$ und $v$ mit den metrischen Koordinaten $a+1/b+1$ oder $a+1/b+1/c+1$. Dieser Knoten hat die gleichen metrischen Koordinaten wie der Knoten an $v$ welcher auch in $S_{n,1}$ ist. Dadurch wäre der Graph $S_{n,1}$ auch nicht getrennt. $\lightning$
\item Angenommen, es gibt einen anderen Knoten $v'$ als $v$ mit der Distanz $2a+k$ zu dem Knoten $v_k$ und den gleichen metrischen Koordinaten wie $v$. Nach den Lemmata \ref{lemnurbifur1} und \ref{lemnurbifur2} ist dieser Fall für eine ungerade Sonne mit zwei Ankerknoten und eine gerade Sonne mit drei Ankerknoten nicht möglich.
%\begin{itemize}
...
\end{proof}
\begin{bem}
Nach Lemmata \ref{sonneerweiterung} und \ref{einelementreichtnicht} ist die metrische Dimension von einer $S_n$ nicht größer als von einer $S_{n,1}$. Und nach Lemma \ref{sun1} ist sie auch nicht kleiner. Also gilt für die metrische Dimension von Sonnengraphen: $$\beta(S_{n,1})=\beta(S_{n,k})=\beta(S_n)$$
...
Durch die Wahl von $v_{\frac{n}{2}-1}$ oder $v_{\frac{n}{2}+1}$ entstehen mindestens zwei Markierungen der Form $a/b$ und $a+1/b+1$ auf dem Kreis, entweder nebeneinander, oder mit dem Abstand $2a+1$. Da jeder Knoten ein Strahlursprung ist, gibt es in dem Graphen einen weiteren Knoten mit der Markierung $a+1/b+1$. Zur Veranschaulichung wird der Graph $S_{6,2}$ betrachtet.
\vspace{-5mm}
\begin{figure}[h!]
		\centering
...
Ein ungerader Sonnengraph $S_n$ mit $n\geq 3$ ist getrennt, wenn der maximale Abstand zwischen zwei Strahlursprüngen mit einem Ankerknoten $max\;dist(r_i,r_j)\leq \frac{n+3}{2}$ mit $r_i,r_j \in R$ und $r_j\neq r_i$.
\end{lem}
\begin{proof}[Beweis:]
Angenommen zwei Knoten sind nicht getrennt. Nach Lemma \ref{lemnurbifur1} sind bei einem ungeraden Sonnengraph zwei Knoten nur dann nicht getrennt wenn sie einen gemeinsamen Bifurkator haben. Es muss einen Strahlursprung mit der maximal möglichen Distanz $\frac{n-1}{2}$ geben und einen Nachbarn $v$ mit einem Strahl ohne Ankerknoten. Damit es einen gemeinsamen Bifurkator gibt darf der kürzeste Weg von $v$ zu allen Ankerknoten höchstens $\frac{n-1}{2}-1$ sein und ist mindestens $1$. Der maximale Abstand zwischen zwei Ankerknoten ist mindestens $n-(\frac{n-1}{2}-1)+1=\frac{n+5}{2}$.
\end{proof}
\vspace{-1mm}
\begin{lem}
\label{gerade_sonne}
...
\vspace{-1mm}
\begin{proof}[Beweis:]
Analog zum Beweis von Lemma \ref{ungerade_sonne} mit dem Unterschied, dass die maximal mögliche Distanz $\frac{n}{2}$ ist und dadurch der maximale Abstand zwischen zwei Ankerknoten mindestens $n-\frac{n}{2}+2=\frac{n+4}{2}$ ist.
\end{proof}

\vspace{-1mm}
\begin{lem}
Ist in einem ungeraden Sonnengraphen $S_n$ mit $n\geq 3$ mindestens ein Ankerknoten oder sind in einem gerade Sonnengraphen $S_m$ mit $m\geq 6$ mindestens zwei Ankerknoten so reicht die Aufnahme von einem weiteren Knoten in die metrische Basis aus um den Graphen zu trennen. 
\end{lem}

\vspace{-1mm}
\begin{proof}[Beweis:]
Der größte Abstand ohne Ankerknoten bei einem ungeraden Sonnengraphen mit mindestens einem Ankerknoten ist $n-1$. Setzt man den Knoten genau in die Mitte von diesem Weg, so schrumpft die maximale Distanz zwischen Ankerknoten auf $\frac{n-1}{2}<\frac{n+3}{2}$.\newline\newline
Die maximale Distanz zwischen mindestens zwei Ankerknoten auf einem geraden Sonnengraphen ist $n-2$, dies ist der Fall wenn es genau zwei Ankerknoten gibt und diese nebeneinander liegen. Positioniert man den Knoten genau in die Mitte des Weges, so ist die neue maximale Distanz $\frac{n-2}{2}<\frac{n}{2}+1$. Ist diese bereits besetzt, kann ein beliebiger Knoten genommen werden.
\end{proof}
...
Sei ein Kreis $C_n$ für $n \geq 3$ mit der Knotenmenge $|V|=\{ c_1, \ldots , c_n \}$ und $n'$ Weggraphen $P_{k'}$ mit $1 \leq n' \leq n-1$ für $2 \leq k' \leq k$ gegeben. Die Knoten mit Grad eins auf dem $i-$ten Weg werden als $v_{i,1}$ und $v_{i,k-1}$ bezeichnet. Durch das Hinzufügen von $n'$ neuen Kanten der Form $\{v_{i,1},c_i\}$ für $1 \leq i \leq n$ entsteht der Sonnengraph $S'_{n',k'}$. Bei mindestens zwei Strahlen unterschiedlicher Länge wird der unvollständige Sonnengraph als $S'_n$ bezeichnet. 
\end{defi}
\vspace{-1mm}
\begin{bem}
\label{bemsonne}
...
\vspace{-1mm}
\begin{proof}[Beweis:]
Um einen unvollständigen Sonnengraphen aus einem Kreis $C_n$ zu erhalten wird mindestens ein neuer Knoten erzeugt und mit genau einem Knoten in dem Graphen $C_n$ verbunden. Dieser Vorgang wird wiederholt, bis der gewünschte unvollständige Graph erzeugt ist. Um einen Sonnengraphen $S_{n}$ aus einem unvollständigen Sonnengraphen zu erhalten, wird mindestens ein Knoten hinzugefügt und mit genau einer Kante verbunden. Nach Lemma \ref{einelementreichtnicht} gilt für die metrische Dimension:
$$\beta(C_n)  \leq \beta(S'_{n}) \leq \beta(C_n) +1 \text{ und } \beta(S'_{n}) \leq \beta(S_n) \leq \beta(S'_{n})+1$$
\end{proof}\vspace{-4mm}
\begin{lem}
\label{usg}
Ist $max\;dist(r_i,r_j)> \lceil \frac{n+2}{2}\rceil$, kann der unvollständige Sonnengraph trotzdem getrennt sein. Ist der Sonnergraph ungerade, so ist er durch zwei Ankerknoten getrennt oder durch drei Ankerknoten, wenn er gerade ist, sofern es keine Strahlen gibt auf allen Knoten $v$, wobei:\newline
$-$ $v$ liegt zwischen den Knoten $r_i$ und $r_{j+\lfloor\frac{n}{2}\rfloor}$ (gezählt auf der Seite wo $r_i$ liegt) \\$-$ $v$ liegt zwischen den Knoten $r_j$ und $r_{i+\lfloor\frac{n}{2}\rfloor}$ (gezählt auf der Seite wo $r_j$ liegt)
\end{lem}
\begin{bem}
Alle Knoten bei welchen es keinen Strahl geben darf, würden sonst Bifurkatoren zwischen allen Ankerknoten, einem Knoten auf dem Strahl und einem Knoten auf dem Kreis sein. Gibt es keine Knoten auf dem Strahl, so gibt es auch keine Bifurkatoren mit der Eigenschaft aus Lemma \ref{Bifurnachbar}. Nach den Lemmata \ref{lemnurbifur1} und \ref{lemnurbifur2} können nur nicht getrennte Knotenpaare existieren, wenn es einen solchen Bifurkator gibt. Der unvollständige Sonnengraph ist damit getrennt.
\end{bem}
\begin{bem}
Nach Lemma \ref{schrankenunvsg} gilt für $n=2k+1$ für $k \in \mathbb{N}$: $$2=\beta(C_n) \leq \beta(S'_{n,k})\leq \beta(S_{n,k})=2$$
Daraus folgt, dass die metrische Dimension eines unvollständigen Sonnengraphen $S'_{n,k}$ mit $n=2k+1$ für $k \in \mathbb{N}$ zwei ist. Für die metrische Dimension für $n=2k+2$ für $k \in \mathbb{N}$ gilt: $$2=\beta(C_n) \leq \beta(S'_{n,k})\leq \beta(S_{n,k})=3$$
...
\begin{figure}[h]
\vspace{-7mm}
\begin{minipage}[l]{160pt}
\centering
\includegraphics[width=130pt]{bilder/ungerade.pdf}
   \caption{Strahllänge $l_k$}
\end{minipage}
\begin{minipage}[r]{260pt}
Zwei Ankerknoten in gegenüberliegenden Strahlen trennen niemals eine unvollständige Sonne. Bei allen anderen Kombinationen gibt es die gegenüberliegenden Blöcke ohne Strahlen, welche sonst den Strahlursprung als Bifurkator aus Lemma \ref{Bifurnachbar} hätten. Die Größe des verbotenen Blocks zwischen den Strahlursprüngen mit den Ankerknoten ist die kürzeste Weglänge $kw(r_1,r_2)-1$. Die verbotene Strahllänge $l_k$ an jedem Knoten aus dem Block berechnet sich aus:
$$l_k=\frac{n-2kw(r_1,r_2)}{2}$$
\end{minipage}
\end{figure}
\begin{table}[htp]
\centering
...
\hline
\multirow{2}{5mm}{Typ}&\multirow{2}{37mm}{Größe der gegenüber-\\liegenden Blöcke ohne\\Strahlen}& \multirow{2}{38mm}{Verbotene Strahllänge}& \multirow{2}{30mm}{Größe des verbo-\\tenen Blocks}\\
&&&\\
\hline\hline
1. Fall&\multirow{3}{20mm}{0}&\multirow{3}{20mm}{1}&\multirow{3}{20mm}{$\frac{n}{2}-2$}\\
\cline{1-1}
\vspace{-6mm}&&&\\
	\includegraphics[width=50pt]{bilder/fall1.pdf}&&&\\
\hline\hline
\vspace{0.3mm}
2. Fall&\multirow{3}{20mm}{1}&\multirow{3}{20mm}{2}&\multirow{3}{20mm}{$\frac{n}{2}-3$}\\
\cline{1-1}
\vspace{-6mm}&&&\\
\includegraphics[width=50pt]{bilder/fall2.pdf}&&&\\
\hline\hline
\vspace{0.3mm}
3. Fall&\multirow{3}{20mm}{2}&\multirow{3}{20mm}{3}&\multirow{3}{20mm}{$\frac{n}{2}-4$}\\
\cline{1-1}
\vspace{-6mm}&&&\\
...
\hline\hline
\vspace{0.3mm}
$\frac{n}{2}-2$. Fall&\multirow{3}{20mm}{$\frac{n}{2}-1$}&\multirow{3}{20mm}{$\frac{n}{2}-2$}&\multirow{3}{20mm}{1=$\frac{n}{2}-\frac{n}{2}+1$}\\
\cline{1-1}
\vspace{-6mm}&&&\\
...
\vspace{-3mm}
\glqq{Haben je zwei Bekannte einen weiteren gemeinsamen Bekannten und sie sind untereinander bekannt, so gibt es eine Person, welche alle anderen kennt}\grqq$\;$. Diese Behauptung kann als ein Graphenproblem definiert werden, wobei Knoten Personen und Kanten Bekanntschaften repräsentieren. Um es zu lösen, wurde von Paul Erdős et al. \cite{Erdos} der Freundschaftsgraph eingeführt. In der Arbeit \glqq{The resolving graph of amalgamation of cycles}\grqq$\;$\cite{amal} wird die metrische Dimension von Freundschaftsgraphen bestimmt. Der Schwerpunkt der Arbeit liegt auf der Anzahl der unterschiedlichen metrischen Basen.
...
\end{lem}
$Beweis\;\;von\;\;Lemma\;\;\ref{mindfreundschaftsgraph}:$
\vspace{-7mm}
\begin{figure}
\vspace{-7mm}
...
Nach Lemma \ref{mindfreundschaftsgraph} ist bekannt, dass die metrische Dimension eines Freundschaftsgraphen $F_{n,3}$ mindestens $n$ ist. Angenommen, alle als $v_{i,2}$ markierten Knoten werden in die metrische Basis aufgenommen. Dann sind sie getrennt. Der Knoten $u$ hat die Distanz eins zu allen Knoten in der metrischen Basis und ist der einzige Knoten mit dieser Eigenschaft. Für jeden Knoten $v_{i,1}$ gibt es genau einen Knoten $v_{i,2}$ mit der Distanz eins. Zu allen anderen Knoten in der metrischen Basis hat jeder Knoten $v_{i,1}$ die Distanz zwei. Alle Markierungen sind eindeutig und der gesamte Graph ist durch die $n$ Knoten getrennt.
\end{proof}
\vspace{-14mm}
~ \linebreak
...
\item  Setzt man einen Ankerknoten $r$ mit der Distanz $dist(u,r)= \frac{n+1}{2}$ so sind die Knoten $v,w$ getrennt. Der Separationsknoten $u$ wird lokal als Ankerknoten aufgefasst. Ein ungerader Kreis ist durch zwei Ankerknoten getrennt. Für den ungeraden Sonnengraphen gilt $\frac{n+1}{2}< \frac{n+3}{2}$. Nach Lemma \ref{ungerade_sonne} ist er getrennt und der ungerade unvollständige Sonnengraph damit auch nach Bemerkung \ref{bemsonne}.
\item Setze den Knoten mit $dist(u,r)= \frac{n}{2}$ und einen beliebig weiteren als Ankerknoten. Für geraden Sonnengraphen gilt $\frac{n}{2}< \frac{n}{2}+1$. Damit ist nach Lemma \ref{gerade_sonne} der Teilgraph getrennt und der ungerade unvollständige Sonnengraph auch nach Bemerkung \ref{bemsonne}.
\end{itemize}
\vspace{-2mm}
\end{proof}
\vspace{-4mm}
Beim geraden Kreis trennt ein Ankerknoten entweder die Knotenpaare in der Komponente oder die Nachbarknoten des Separationsknoten. Sonst trennt ein (oder zwei) Ankerknoten gleichzeitig beides. Damit ist die Reihenfolge in der die Knoten getrennt werden unwichtig.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newpage
\vspace{-2mm}
...
\newpage
\section{Der Algorithmus zur Berechnung der metrischen Dimension von Kaktusgraphen}
...
\vspace{-3mm}
   ...
   \label{kaktus4}
  	 \end{figure}
\newpage In Abbildung \ref{kaktus4} wurden alle ZZK durch Bäume mit dem Algorithmus \ref{algreplace} ersetzt. Weiterhin wurden die Amalgamationsknoten überprüft, so dass nur noch Amalgamationsknoten mit mindestens zwei nicht getrennten Nachbarn in der Menge sind.\newline
\vspace{-1mm}
  	   	 \begin{figure}[h!]
		\centering
...
\STATE Ändere die Auflistung der zweifachen Zusammenhangskomponenten $x$ von Kanten zu Knoten ohne Wiederholungen, speichere für jeden Knoten $v$ zusätzlich jede ZZK, in welcher er vorkommt, in einer Liste $v.ZZK$ und zähle die Anzahl der Knoten und speichere die Werte in $x.Knoten$;
...
\IF{($x.Knoten==S[x]$)}{\STATE return $S(x)$;}\ELSIF{($x.Knoten==K[x]$)}{\STATE return $K(x)$;}\ELSE{\IF{($Anker[x] \leq 2$ oder $Anker[x] \geq \frac{n-3}{2}$)}{
		\IF{($(n\; \textbf{mod} \; 2)==1$ oder $n==4$)}
		{		 \STATE return $USU(x)$;
			}
		\ELSIF{($(n\; \textbf{mod} \; 2)==0$)}{
		\IF{($Anker[x]==0$)}{\STATE return $USG0(x)$;}
			\ELSE{\STATE return $USG1(x)$;}
			\ENDIF
		}\ENDIF}
	\ELSE{\STATE  return $US(x)$;
	}\ENDIF}\ENDIF
}\ENDFOR
\vspace{2mm}
\end{algorithmic}
...
\ENDIF
\FORALL{($x \in v.ZZK$)}{
\IF{($Anker[x]==1$ und $(n\; \textbf{mod} \; 2)==1$) oder\\$\;\;\;\;$($dist(v,x) \leq \lceil\frac{n}{2}\rceil$)%oder\\$\;\;\;\;$($x==C$)
...
\ELSIF{($3 \leq Anker[x]<\frac{n-3}{2}$)}{\IF{($dist[x]\leq \lfloor\frac{n+3}{2}\rfloor$)}{\STATE $MD^+(x)=0$;}\ELSE{\STATE $MD^+(x)=1$; $insert(x \text{ in }  MD-)$;}\ENDIF
}\ELSIF{($Anker[x]\geq \frac{n-2}{2}$ und $(n\; \textbf{mod} \; 2)==0$)} \STATE $MD^+(x)=0$;
\ELSIF{($Anker[x]\geq \frac{n-3}{2}$ und $(n\; \textbf{mod} \; 2)==1$ und $n>5$)}{ \STATE $MD^+(x)=0$;}\ENDIF
\vspace{2mm}
\end{algorithmic}
...
\begin{algorithmic}[1] 
\vspace{2mm}
\IF{($Anker[x]==0$)} \STATE $MD^+(x)=2$;\ELSIF{($Anker[x]==1$)} \STATE $MD^+(x)=1$;\ELSIF{($Anker[x]\geq \frac{n-3}{2}$)} \STATE $MD^+(x)=0$;\ELSIF{($Anker[x]==2$)} \IF{($dist(x)==\frac{n+1}{2}$ oder $dist(x)==\frac{n+3}{2}$)}{\STATE $MD^+(x)=0$;}\ELSIF{(Jeder Knoten $v \in x \setminus \{v_1,v_2,v_{1+\frac{n-1}{2}},v_{1+\frac{n+1}{2}},v_{2+\frac{n-1}{2}},v_{2+\frac{n+1}{2}}\}$) hat\\ $Klasse[v]=0$)}{\STATE $MD^+(x)=0$;} \ELSE{\STATE $MD^+(x)=1$;\STATE $insert(x \text{ in }  MD-)$;} \ENDIF \ENDIF
\end{algorithmic}
\label{algberechnungungeradesonne1}
\end{algorithm}
\vspace{-2mm}
~\linebreak 
Algorithmus \ref{algberechnungungeradesonne1} überprüft die \glqq{einfachen}\grqq$\;$Fälle, dabei wird eine zusätzliche \textsc{for}-Schleife benötigt, für den Fall, dass zwei $A$-Knoten vorhanden sind und diese eine ZZK vom Typ $S$ nicht trennen würden. Dafür wird mittels Tiefensuche einmalig eine ZZK überprüft, ob bestimmte Knoten der Klasse $0$ angehören.\\
\vspace{-2mm}
\begin{algorithm}
\caption{$US(x)$}
\begin{algorithmic}[1]
\IF{($dist[x]\leq \lfloor\frac{n+3}{2}\rfloor$)}{\STATE $MD^+(x)=0$;}\ELSIF{(Jeder Knoten $v$ auf dem kürzeren Weg zwischen $v_1$ und$v_{2+\frac{n}{2}}$ und zwischen $v_2$ und $v_{1+\frac{n}{2}}$ hat $Klasse[v]=0$)}{ \STATE $MD^+(x)=0$;}
...
Der Algorithmus \ref{algberechnunggeradesonne1} läuft über die ZZK in Zeile 2 und 6 und überprüft in konstanter Zeit ob der gegenüberliegende Knoten ebenfalls der Klasse $0$ zugewiesen wurde. In Zeile 7 und 15 kann ein weiterer Durchlauf über die ZZK benötigt werden, um zu überprüfen, ob die zwei ausgewählten Knoten die ZZK trennen.\newline
\vspace{-3mm}
\begin{algorithm}
\caption{$USG1(x)$}
\begin{algorithmic}[1]
...
\IF{($Anker[x]==1$ und $n>4$)}
	\STATE Finde eine maximale zusammenhängende Menge links $M$ mit $|M|=s$ und eine rechts $M'$ mit $|M'|=s'$ mit 			$Klasse[v]=0$;
	\IF{($s \geq \frac{n}{2}-2$ oder $s'\geq \frac{n}{2}-2$)}{ \STATE $MD^+(x)=1$;}\ELSE{
	\STATE Finde maximale gegenüberliegende zusammenhängende Menge links $M$ mit $|M|=s$ und dem Ankerknoten $v_{1-s-1+\frac{n}{2}}$ und rechts $M'$ mit $|M'|=s'$ dem Ankerknoten $v_{1+s'+1+\frac{n}{2}}$ mit $Klasse[v]=0$;
	\IF{($\{v_1, v_{1-s-1+\frac{n}{2}}\}\text{ oder }\{v_1, v_{1+s'+1+\frac{n}{2}}\}$ die ZZK trennen)}{\STATE $MD^+(x)=1$;}\ELSE{\STATE $MD^+(x)=2$;\STATE $insert(x \text{ in }  MD-)$;}\ENDIF
	}\ENDIF
\ELSIF{($Anker[x]==2$ und $n>4$)} 
	\IF{($\{v_1, v_{2}\}$ die ZZK trennen)}{ \STATE $MD^+(x)=0$;}
	\ELSE{\STATE $MD^+(x)=1$;\STATE $insert(x \text{ in }  MD-)$;}\ENDIF 
\ELSIF{($Anker[x]\geq \frac{n}{2}-2$)}  \STATE $MD^+(x)=0$;\ENDIF
\vspace{2mm}
\end{algorithmic}
...
\begin{figure}[h!]
\centering
\includegraphics[width=420pt]{bilder/algsonngeradea02.pdf}
...
Es wird überprüft, ob an diesem Knoten neben seinen Nachbarn in der ZZK nur ein einfacher Weg ist. Dann ist die Länge des Weges die Klasse $i$ des Knotens. Oder wenn es kein einfacher Weg ist, so ist die Klasse vom Knoten $A$.
\end{itemize}
\end{bem}
\end{comment}
\vspace{-12mm}
...
\begin{itemize}
\item[Typ $K$]
Die metrische Dimension von einem Kreis ist zwei. In der Arbeit \glqq{Resolvability and the upper dimension of graphs}\grqq$\;$\cite{upper} wird die Oberdimension von Kreisen behandelt. Die Oberdimension ist die minimale Anzahl von Knoten, so dass jede Menge eine metrische Basis ist.\newline
...
\begin{lem}
Sei $C_{3,j}$ ein beliebiges $C_{3}$-Blatt. Jede metrische Basis muss mindestens einen der folgenden Knoten $\{v_{j,1},v_{j,2},v_{j,3},v_{j,4}\}$ beinhalten.
\end{lem}
\begin{proof}[Beweis:]~
\par
\vspace{-2mm}
...
Da der Knoten $b_3$ ein Separationsknoten ist, ist die Distanz von diesem Knoten zu $b_3$ echt kleiner als zu jedem anderen Knoten in dem ausgewählten $C_{3,j}$. Die Markierung von $b_3$ sei o.B.d.A. an der ersten Position $a$. Sein einziger Nachbar $c_3$ bekommt die Markierung $a+1$ an der ersten Position. Die anderen zwei Knoten $c_1$ und $c_2$ auf dem $C_3$ bekommen die Markierung $a+2$ an der ersten Position und ihre zwei Nachbarn $b_1$ und $b_2$, die Markierung $a+3$ an der ersten Position. Somit erhalten die Knotenpaare $b_2$ und $b_3$ und $c_2$ und $c_3$ unterschiedliche Markierungen und sind getrennt.
\begin{figure}[h!]
		\centering
...
Der Knoten $u$ hat entweder den Grad eins und ist ein Blatt, welches ein Nachbar eines $C_3$ ist, oder den Grad drei und ist Teil eines festen $C_{3}$. Da der Graph zusammenhängend ist, gibt es in beiden Fällen eine Kante $\{x,y\}$ in dem gleichen $C_{3,j}$ mit der Eigenschaft $x$ Teil von diesem $C_{3}$ und $y$ nicht in diesem diesem $C_{3}$. Außerdem gilt $dist(v,y) < dist(v,x)$. Da $y$ in dem $C_3$ ist kann $y$ nicht der Knoten $v$ sein und durch die Lage kann $y$ kein Blatt sein. Nach Lemma \ref{bkb} sind $x$ und $y$ zwei Separationsknoten, und die beiden nicht zusammenhängenden Graphen, welche durch das Entfernen des jeweiligen Knoten entstehen, beinhalten nach Lemma \ref{bkb2} mindestens ein Element aus der metrischen Basis.\\
...
\begin{algorithmic}
\STATE 1. Bestimme alle zweifachen Zusammenhangskomponenten mit mindestens 3 Kanten und\\$\;\;\;\;$ordne jeden Knoten jeder Zusammenhangskomponente zu einer der drei Klassen\\$\;\;\;\;\{0,i,A\}$ mit $i \geq 1$;
...
Die ersten beiden wurden in \glqq{Resolvability and the upper dimension of graphs}\grqq$\;$\cite{upper} eingeführt. Die Oberdimension eines Graphen ist die größte Menge an Knoten, so dass kein Knoten überflüssig ist und die Menge eine trennende Menge ist. Die trennende Zahl ist die Größe der kleinsten Menge an Knoten, so dass bei beliebiger Wahl von Knoten aus dem Graphen, jede Menge dieser Größe eine trennende Menge ist. Sie zu kennen ist nützlich bei Algorithmen, da aus einer Anzahl der Knoten geschlossen werden kann, dass der Graph getrennt ist, ohne die Position der Knoten zu kennen.
\vspace{-1.5mm}\newline\newline 
In der Arbeit \glqq{On the partition dimension of a graph}\grqq$\;$\cite{partit} wurde aus der metrischen Dimension die Partitionsdimension (PD) entwickelt. Während bei der metrischen Dimension nur einzelne Knoten in die Basis aufgenommen werden, dürfen bei der PD Mengen von Knoten aufgenommen werden. Weiterhin muss die gesuchte Partition jeden Knoten trennen und den gesamten Graphen in Mengen aufteilen. Die PD des Graphen $pd(G)$ ist die kleinste Anzahl von Mengen. Die Distanz eines Knotens zu einer Menge ist immer die kürzeste Distanz zu einem Knoten aus der Menge.\newline
Die metrische Dimension um eins erhöht ist eine obere Schranke für die PD\cite{partit}. Es gibt aber auch Graphen mit deutlich kleinerer PD als metrischer Dimension, z.B. den Graphen aus der Arbeit \glqq{Discrepancies between metric dimension and partition dimension of a connected graph}\grqq$\;$\cite{disc}. Viele Fragestellungen sind bereits gelöst worden, aber einige Berechnungen sind noch offen, wie z.B. die Bestimmung der PD bei Bäumen. Es gibt außerdem viele Abwandlungen der PD, z.B. zusammenhängende PD aus \glqq{Connected partition dimensions of graphs}\grqq$\;$\cite{con}, die \glqq{Locating-Chromatic Number}\grqq$\;$aus \glqq{The locating-chromatic number of a graph}\grqq$\;$\cite{loc} und die \glqq{Forcing}\grqq$\;$Dimension aus \glqq{The forcing dimension of a graph}\grqq$\;$\cite{forcing}.
\vspace{-4mm}
\subsection{Algorithmen zur Berechnung der metrischen Dimension}
\vspace{-2mm}
Die Autoren von der Arbeit \glqq{On the Complexity of Metric Dimension}\grqq$\;$\cite{aussenplanar} geben einen Algorithmus zur Bestimmung der metrischen Dimension von einem außenplanaren Graphen in Polynomialzeit an. Der Algorithmus arbeitet mit dem verallgemeinerten Dualbaum des Graphen und überprüft mittels dynamischer Programmierung bestimmte Eigenschaften von Teilbäumen. 
\vspace{-1mm}
\newline\newline
Die metrische Dimension von blätterlosen und nicht antipodalen Block-Kaktusgraphen wird in der Arbeit \glqq{Metric Dimension of antipodal and pendant-free block-cactus graphs}\grqq$\;$\cite{cactusblock} analysiert. Die Kaktusgraphen werden durch vollständige Graphen zusätzlich zu Kreisen erweitert, aber auch stark eingeschränkt, indem Wege und Bäume an Kreisknoten verboten werden. In geraden Kreisen werden gegenüberliegende Separationsknoten nicht erlaubt. Dadurch gibt es noch weniger Fallunterscheidungen. Insbesondere werden in der Arbeit nur Anforderungen an die Anzahl der Ankerknoten in bestimmten Teilgraphen gestellt.
\vspace{-5mm}
\section{Offene Fragen und zukünftige Forschung}
\vspace{-2mm}
Der Algorithmus zur Berechnung der metrischen Dimension von Kaktusgraphen könnte durch das Zulassen von vollständigen Graphen, Gittern, vollständig bipartiten Graphen oder auch Radgraphen als zweifache Zusammenhangskomponenten erweitert werden. Dafür müsste die metrische Dimension von diesen Graphen mit Wegen erforscht werden.\\ Die Gittergraphen mit Wegen der Länge eins an den Knoten wurden in \glqq{Corrections to the article: The metric dimension of graph with pendant edges}\grqq$\;$\cite{grid} erforscht, die metrische Dimension von solchen Graphen ist drei. Es ist hier notwendig, zunächst das Ergebnis auf beliebige Wege zu erweitern und dann die Graphen zu finden, welche mit Wegen versehen sind, aber noch die metrische Dimension zwei haben. Die anderen Graphen erfordern eine detaillierte Untersuchung.
\vspace{-1.5mm}\newline\newline
